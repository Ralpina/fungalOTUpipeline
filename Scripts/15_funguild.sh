...
# let's also prepare a concatenated file to allow counting unique SHs:
cat guilds.SH.txt denovo.guilds.SH.txt > allguilds.txt
...
grep "Probable" allguilds.txt | grep "Ectomycorrhizal" > allEctoguilds.txt
...
echo "Total number of Ectomycorrhizal sequences (de novo and not)"
wc -l allEctoguilds.txt 
echo "Total number of de novo Ectomycorrhizal sequences (centroids)"
wc -l Ectodenovoguilds.txt 
# the above should actually be considered separately if we assume that two de novo centroids with the same SH could be different taxa, although with some similarity
# in fact, remember that de novo were searched against UNITE with a low similarity threshold
...
echo "Number of different SHs in all Ectomycorrhizal sequences (de novo and not)"
cut -f 18 allEctoguilds.txt | sort | uniq | wc -l
# the above can be used as a rough indication but it shouldn't be used to count de novo sequences, as we assume that each de novo is self-standing because of the similarity threshold used

# to extract the non-ecto sequences, we do the opposite as we did above:
grep "Probable" allguilds.txt | grep -v "Ectomycorrhizal" > non_ecto_allguilds.txt
...
echo "Number of different SHs in all non-ectomycorrhizal sequences (de novo and not)"
cut -f 18 non_ecto_allguilds.txt | sort | uniq | wc -l

# the above can be used as a rough indication but it shouldn't be used to count de novo sequences, as we assume that each de novo is self-standing because of the similarity threshold used

#all other sequences, mostly unidentified
echo "Total number of other sequences, with FUNGuild assignments possible or not available"
grep -v "^OTU" allguilds.txt | grep -v "Probable" | wc -l
#unique SHs in these
echo "Total number of different SHs with FUNGuild assignments possible or not available"
grep -v "^OTU" allguilds.txt | grep -v "Probable" | cut -f 18 | sort | uniq | wc -l
